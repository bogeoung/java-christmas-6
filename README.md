...
- 사용자에게 방문 날짜를 입력받는다.
  - 1이상 31이하의 숫자가 아닌 경우 에러를 출력한다 → `[ERROR] 유효하지 않은 날짜입니다. 다시 입력해 주세요.`

### 2. 사용자에게 주문할 메뉴와 개수를 입력받는다.

- `주문하실 메뉴를 메뉴와 개수를 알려 주세요. (e.g. 해산물파스타-2,레드와인-1,초코케이크-1)` 문구를 출력한다.
- 사용자에게 메뉴와 개수를 입력받는다.
  - 아래 예외 케이스에 대해서는 에러를 출력한다. → `[ERROR] 유효하지 않은 주문입니다. 다시 입력해 주세요.`
    - 메뉴판에 없는 메뉴를 입력하는 경우
    - 메뉴 형식이 예시와 다른 경우
    - 중복 메뉴를 입력한 경우
    - 주문 개수가 20개를 초과한 경우
...
- `12월 [입력날짜]일에 우테코 식당에서 받을 이벤트 혜택 미리 보기!` 문구를 출력한다.
- `<주문 메뉴>` 문구를 출력한다.
- ‘메뉴명 n개’형식으로 주문한 메뉴들을 출력한다.

### 4. 혜택내역을 출력한다.

- 할인 전 총 주문 금액을 출력한다.
  - `<할인 전 총주문 금액>` 문구를 출력한다.
  - 주문한 음식의 총 주문 금액을 계산하여 출력한다.
- 증정 메뉴를 출력한다.
  - `<증정 메뉴>` 문구를 출력한다.
  - 총 주문 금액이 12만원 이상인 경우 샴페인을 무료로 증정한다.
  - 총 주문 금액이 12만원 미만인 경우 증정품은 존재하지 않는다.
- 혜택 내역을 출력한다.
  - 총 주문 금액이 10,000원 이상인 경우에만 혜택이 적용된다.
  - `<혜택 내역>` 문구를 출력한다.
  - 크리스마스 디데이 할인을 계산하여 출력한다.
    - 1~25일에만 할인이 제공된다.
    - 할인 금액은 1,000원으로 시작하여 날마다 할인 금액이 100원씩 증가한다.
  - 평일(일~목)이라면 평일할인을 계산하여 출력한다.
    - 디저트 메뉴를 메뉴 1개당 2023원 할인한다.
  - 평일이 아니라면(금,토) 주말 할인을 계산하여 출력한다.
    - 메인 메뉴를 메뉴 1개당 2023원 할인한다.
  - 특별할인을 계산하여 출력한다.
    - 이벤트 달력에 별이 있으면 총주문 금액에서 1,000원을 할인한다.
    - 이벤트 달력에 별이 있는 날은 모든 일요일과, 12/25일이다.
  - 증정 이벤트 내역을 출력한다.
    - 증정 메뉴가 존재한다면 샴페인 값을 증정 이벤트 혜택으로 간주한다.

### 5. 이벤트 배지를 출력한다.이벤트 배지를 출력한다.
